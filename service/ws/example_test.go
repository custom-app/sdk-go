...
import (
	...
	ws2 "github.com/loyal-inform/sdk-go/service/job/ws"
	...
	"github.com/loyal-inform/sdk-go/service/ws/pool"
...
func (s *Service) Start() error {
	...
	s.poolWorkers = make([]*ws2.PrivateWorker, 4)
	for i := range s.poolWorkers {
		s.poolWorkers[i] = ws2.NewPrivateWorker(s.pool.GetQueue(), s.handleMessage)
...
func Example() {
	s, err := ws.NewServer(&Service{}, "0.0.0.0:1337")
	if err != nil {
		log.Panicln(err)
	}
	go func() {
